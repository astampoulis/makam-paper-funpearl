...
we can catch some errors that way -- for example this erroneous `fold` is still well-typed
...
issues related to polymorphic types that have not allowed me to test there.
...
define a new `nat` kind and specify it in the type of `dbind`, it would be fine, but as
