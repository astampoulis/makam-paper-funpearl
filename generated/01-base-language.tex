...
\hero{HAGOP} OK, let's just start with the simply typed lambda calculus to see
how this works. Let's define just the basics, application, lambda
abstraction and the arrow type.

\hero{ROZA} Right. We will first need to define the two meta-types for these
two sorts:
...
\hero{HAGOP} Oh, so \texttt{type} is the reserved keyword for the meta-level
kind of types, and we'll use \texttt{typ} for our object-level types?

\hero{ROZA} Exactly. And let's do the easy constructors first:
...
\hero{HAGOP} So we add constructors to a type at any point, we do not list
...
\hero{ROZA} Yes, functions at the meta-level are parametric, so they
...
\hero{HAGOP} I see. And how about the typing judgement,
\(\Gamma \vdash e : \tau\) ?

\hero{ROZA} Let's add a predicate for that. Only, no \(\Gamma\), there is an
...
\hero{HAGOP} Let me see if I can get the typing rule for application. I know
...
\hero{ROZA} Yes! That's exactly right. Makam uses capital letters for
unification variables.

\hero{HAGOP} I will need help with the lambda typing rule though. What's the
equivalent of extending the context as in \(\Gamma, x : \tau\) ?

\hero{ROZA} Simple, we introduce a fresh constructor for terms, and a new
typing rule for it:
...
\hero{HAGOP} Hmm, so \texttt{x:term\ \ensuremath{\to}} introduces the fresh
...
\hero{ROZA} Yes. Note that the introductions are locally scoped, so they are
only into effect for the recursive call to \texttt{typeof}.

\hero{HAGOP} Makes sense. So do we have a type checker already? Can we run
queries?

\hero{ROZA} We do! Observe:
...
\hero{HAGOP} Cool! So underscores for unification variables we don't care
...
\hero{ROZA} Well you were missing the occurs-check. \lamprolog unification
includes it:
...
\hero{HAGOP} Right. So let's see, what else can we do? How about adding tuples
to our language? Can we use something like a polymorphic list?

\hero{ROZA} Sure, \lamprolog has polymorphic types and higher-order
predicates:
...
\hero{HAGOP} Nice! I guess that's why you wanted to go with \lamprolog for
doing this instead of LF, since you cannot use polymorphism there?

\hero{ROZA} Indeed. We will see, once we figure out what our language should
...
\hero{HAGOP} Sounds good. So, for tuples, this should work:
...
\hero{ROZA} Yes, and we can use syntactic sugar for \texttt{cons} and
\texttt{nil} too:
...
\hero{HAGOP} So how about evaluation? Can we write the big-step semantics too?

\hero{ROZA} Why not? Let's add a predicate and do the two easy rules:
...
\hero{HAGOP} OK, let me try my hand at the beta-redex case. I'll just do
...
\hero{ROZA} Exactly! See, I told you this would be easy!
